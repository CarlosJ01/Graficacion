...
void figura01(Mat fgr,int a){
	...
	for (grd = 0;grd<(2*PI);grd+=0.01){
		...
		fnd=copiar(fgr);
		circle(fnd,Point(x,y),10,CV_RGB(0,0,0),-1,3);
...
Mat copiar(Mat img){
	Mat copi = Mat(img.rows,img.cols,CV_8UC3,CV_RGB(255,255,255));
	
	for(int i=0;i<img.rows;i++)
		for(int j=0;j<img.cols;j++)
			copi.at<Vec3b>(i,j)=img.at<Vec3b>(i,j);
